				...
				if not status then
					exports["sandbox-base"]:ServerCallback("Hospital:Respawn", {}, function(bedId)
						if bedId ~= nil then
							...
							exports["sandbox-hud"]:Notification("error", "Unable To Respawn Yet, Please Wait")
						end
					end)
				else
	...
	CreateThread(function()
		while LocalPlayer.state.loggedIn do
			...
			elseif LocalPlayer.state.wasOnPainKillers then
				...
				-- SetPedToRagdoll(LocalPlayer.state.ped, 1500, 2000, 3, true, true, false)
				-- exports["sandbox-hud"]:Notification("custom", Config.Strings.PainKillersExpired, 5000, "pills", Config.NotifStyle)
			end
			...
			elseif LocalPlayer.state.wasOnDrugs then
				...
				-- SetPedToRagdoll(LocalPlayer.state.ped, 1500, 2000, 3, true, true, false)
				-- exports["sandbox-hud"]:Notification("custom", Config.Strings.AdrenalineExpired, 5000, "pills", Config.NotifStyle)
			end
