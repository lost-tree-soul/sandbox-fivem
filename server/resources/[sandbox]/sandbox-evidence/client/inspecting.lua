...
function StartInspecting()
	if not inspecting then
		...
		local NotifyStr = string.format("Camera - Press %s to exit", exports["sandbox-kbs"]:GetKey("emote_cancel"))
		if LocalPlayer.state.onDuty == "police" then
			NotifyStr = string.format("Camera - Press %s to take a photo",
					exports["sandbox-kbs"]:GetKey("secondary_action"))
				.. "<br/>"
				.. string.format("Camera - Press %s to exit", exports["sandbox-kbs"]:GetKey("emote_cancel"))
		end
						...
						if #(GetEntityCoords(LocalPlayer.state.ped) - v.coords) <= 8.0 then
							if v.type == "paint_fragment" then
								...
								DrawMarker(
									...
									evidenceMarkers[v.type].scale,
									evidenceMarkers[v.type].scale,
									evidenceMarkers[v.type].r,
									evidenceMarkers[v.type].g,
									evidenceMarkers[v.type].b,
									evidenceMarkers[v.type].a,
									0,
									1,
									0,
									0
	...
	if #localEvidence > 0 then
		...
		if hitting and endCoords then
			...
			if closest and lastDist <= 1.75 then
				...
				exports["sandbox-hud"]:Notification("error", "Not Close Enough to Any Evidence")
			end
		end
	end
end
